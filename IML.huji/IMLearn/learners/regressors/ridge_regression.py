...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        Sigma_denom = Sigma * Sigma + self.lam_
        Sigma_lam = np.concatenate([np.diag(Sigma / Sigma_denom), np.zeros([V_trans.shape[0], U.shape[0] - V_trans.shape[0]])], axis=1)
        self.coefs_ = V_trans.T@Sigma_lam@U.T@y
