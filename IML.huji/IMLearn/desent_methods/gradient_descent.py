...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        weight_t = f.weights_
        weight_average = f.weights_
        min_weight = np.inf
        for t in range(1, self.max_iter_ + 1):
            f.weights_ = weight_t - self.learning_rate_.lr_step(t) * \
                         f.compute_jacobian()
            weight_average = (weight_average * t + f.weights_)/(t + 1)
            min_weight = f.weights_ if np.all(f.weights_ < min_weight) else min_weight
            delta = np.linalg.norm(f.weights_ - weight_t)
            self.callback_(gd=self, weights=f.weights_,
                            values=f.compute_output(),
                            derivatives=f.compute_jacobian(), iteration=t,
                            learning_rate=self.learning_rate_.lr_step(t),
                            delta=delta)
            if delta < self.tol_:
                break
            weight_t = f.weights_
