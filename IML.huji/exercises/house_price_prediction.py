...
def feature_evaluation(X: pd.DataFrame, y: pd.Series, output_path: str = ".") -> NoReturn:
    ...
    for column in X:
        ...
        fig.update_layout(title_text=f"$\\text{{Pearson Correlation of {X[column].name.title()} Feature and Response is {pearson_corr}}}$", title_x = 0.5)
        fig.update_xaxes(title_text=f"{X[column].name.title()}")
        fig.update_yaxes(title_text="Response")
        # fig.write_image(f"{output_path}/{X[column].name.title()}_feature.png")
...
if __name__ == '__main__':
    ...
    processed_df = load_data("C:\\University4\\CS\\Year3\\IML\\IML.HUJI\\datasets\\house_prices.csv")
    ...
    fig = go.Figure(data=[go.Scatter(x=x, y=mean_pred, mode="markers+lines", name="Mean Prediction", line=dict(dash="dash"), marker=dict(color="green", opacity=.7)),
                        go.Scatter(x=x, y=np.array(mean_pred)-2*np.sqrt(var_pred), fill=None, mode="lines", line=dict(color="lightgrey"), showlegend=False),
                        go.Scatter(x=x, y=np.array(mean_pred)+2*np.sqrt(var_pred), fill='tonexty', mode="lines", line=dict(color="lightgrey"), showlegend=False)])
    fig.update_layout(title_text="$\\text{Mean Loss by Percentage of Train}$")
    fig.update_xaxes(title_text="$\\text{Percentage of Train Set}$")
    fig.update_yaxes(title_text="$\\text{Mean Loss}$")
